            ...
            <div class="row">
                <div class="col-md-6"> <!-- This div takes up half the width on the left -->
                    <div style="margin-top: 30px;">
                        <a href="{{ route('viewowner') }}" class="btn btn-primary ml-0 mb-3">Back to Home</a>
                    </div>
                </div>
    ...
    {{-- <div class="modal fade" id="formModal" tabindex="-1" role="dialog" aria-labelledby="formModalLabel" aria-hidden="true"> --}}
        ...
        <div class="modal-header">
            <h3 class="modal-title" id="formModalLabel" style="font-size: 22px; width: 400px; height: 50px; background-color: #007bff; color: #ffffff; text-align: center; padding: 10px; border-radius: 30px; box-shadow: 0 0 20px rgb(0, 0, 0);">Emp Update Form</h3>
    ...
    const regNo = urlParams.get("login_id");
    if (regNo) {
    ...
    searchButton.addEventListener("click", function() {
        ...
        window.location.href = '/view_update?login_id=' + searchTerm;
    ...
    <form method="post" id="editEmpform" >
        @csrf
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-6">
                    <input type="text" name="emp_name" class="form-control" id="email" value="{{ $user->emp_name ?? '' }}" required >
                </div>
            </div>
            <div class="form-group row">

                <div class="col-sm-6">
                    <input type="hidden" name="emp_id" class="form-control" id="email" value="{{ $user->emp_id ?? '' }}" required readonly style="cursor:not-allowed">
                </div>
            </div>
            <div class="form-group row">
                ...
                <div class="col-sm-6">
                <input type="text" name="password" class="form-control" id="email" value="{{ $user->password ?? '' }}" required >
                </div>
            </div>
            <div class="form-group row">
                <label for="email" class="col-sm-6 col-form-label">
                ...
                <div class="col-sm-6">
                <input type="text" name="em_loginid" class="form-control" id="email" value="{{ $user->em_loginid ?? '' }}" required >
                </div>
            </div>
            <div class="form-group row">
                <label for="email" class="col-sm-6 col-form-label">
                ...
                <div class="col-sm-6">
                <input type="text" name="designation" class="form-control" id="email" value="{{ $user->designation ?? '' }}" required readonly style="cursor:not-allowed">
                </div>
            </div>
            <div class="form-group row">
                <label for="email" class="col-sm-6 col-form-label">
                ...
                <div class="col-sm-6">
                <input type="text" name="contact" class="form-control" id="email" value="{{ $user->contact  ?? '' }}" required >
                </div>
            </div>
            <div class="form-group row">
                <label for="email" class="col-sm-6 col-form-label">
                ...
                <div class="col-sm-6">
                <input type="text" name="cnic" class="form-control" id="email" value="{{ $user->cnic  ?? '' }}" required >
                </div>
            </div>
                <div class="form-group row">
                    <label for="email" class="col-sm-6 col-form-label">
                    ...
                    <div class="col-sm-6">
                    <input type="text" name="status" class="form-control" id="email" value="{{ $user->status ?? '' }}" required >
                    </div>
                </div>
                ...
                <div class="form-group row">
                    <label for="email" class="col-sm-6 col-form-label">Role:</label>
                    <div class="col-sm-6">
                        @if(!empty($user->role))
  ...
  <script>
    $(document).ready(function() {
        $('#editEmpform').submit(function(event) {
            ...
            // AJAX POST request
            $.ajax({
                url: '{{ route("edit_emp") }}', // Replace with your Laravel route
                type: 'POST',
                data: formData,
